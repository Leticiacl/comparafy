...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const addItem = async (listId: string, item: NewItem) => {
    const uid = await ensureAuth(); // garante auth antes de escrever
    const newId = await addItemToListService(uid, listId, item);
    await fetchUserData(); // refetch para refletir
    return newId;
  };
  ...
  const savings = lists.reduce((sum, l) => {
    const itens = l.itens || [];
    return (
      sum + itens.reduce((acc, i) => (i.comprado ? acc + i.preco : acc), 0)
    );
  }, 0);
  ...
  const renamePurchaseInContext = async (
    purchaseId: string,
    newName: string
  ) => {
    const userId = getUid();
    ...
    setPurchases((prev) =>
      prev.map((p) => (p.id === purchaseId ? { ...p, name: newName.trim() } : p))
    );
  };
  ...
  const deletePurchaseItemInContext = async (
    purchaseId: string,
    index: number
  ) => {
    const userId = getUid();
  ...
  const appendItemsToPurchaseById = async (
    purchaseId: string,
    items: PurchaseItem[]
  ) => {
    const userId = getUid();
  ...
  const appendItemsToPurchaseInContext = async (
    items: PurchaseItem[]
  ): Promise<string | null> => {
    const userId = getUid();
    if (!userId) return null;
  ...
  // Garante estar autenticado antes de buscar dados
  useEffect(() => {
    (async () => {
      await ensureAuth();
  ...
  return (
    <DataContext.Provider
      value={{
        ...
        fetchItems,
        addItem,
        addItemToList: (listId: string, item: NewItem) => addItem(listId, item), // alias
        updateItem,
        toggleItem,
        deleteItem,
