...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-between min-h-screen bg-white dark:bg-black p-6">
      <div className="w-full text-center mt-8">
        {currentStep === 0 ? (
          ...
          <img src="/COMPARAFY.png" alt="Logo" className="h-10 object-contain mx-auto" />
        )}
      </div>

      <div className="flex flex-col items-center justify-center flex-grow w-full max-w-sm">
        {currentStep > 0 && (
          <div className="flex justify-center items-center mb-8 w-32 h-32 rounded-full bg-yellow-50 dark:bg-yellow-900/20">
          ...
          {steps.map((_, index) => (
            <div
              ...
              className={`w-2 h-2 rounded-full ${
                index === currentStep ? 'bg-yellow-500' : 'bg-gray-300 dark:bg-gray-700'
              }`}
            />
          ))}
        </div>
      ...
      <div className="w-full flex flex-col gap-4 mb-8">
        <button
          onClick={handleNext}
          className="w-full bg-yellow-500 text-black font-medium py-3 rounded-lg flex items-center justify-center"
        >
