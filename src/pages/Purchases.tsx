...
const Purchases: React.FC = () => {
  ...
  return (
    <main className="mx-auto w-full max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl bg-white px-4 md:px-6 pt-safe pb-[88px]">
      ...
      {/* CTA largo igual Listas/Início */}
      <Link
        to="/purchases/new"
        className="mb-4 block w-full rounded-2xl bg-yellow-500 px-4 py-3 text-center font-semibold text-black shadow hover:brightness-95"
      >
        + Nova compra
      </Link>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
        ...
        {fromList.map((p) => (
          <Card
            ...
            onRename={(id, name) => { setRenameId(id); setRenameValue(name || ""); }}
...
}> = ({ id, name, market, createdAt, total, count, onRename, onDelete }) => {
  const navigate = useNavigate();
          ...
          <Menu as="div" className="absolute right-1 top-1">
            ...
            >
              <Menu.Item>
                {({ active }) => (
                  <button
                    onClick={(e) => { e.stopPropagation(); id && onRename(id, name || ""); }}
              ...
              <Menu.Item>
                {({ active }) => (
                  <button
                    onClick={(e) => { e.stopPropagation(); id && onDelete(id); }}
