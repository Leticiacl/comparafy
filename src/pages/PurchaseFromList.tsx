...
const PurchaseFromList: React.FC = () => {
  ...
  }, [list, nameTouched]); // eslint-disable-line react-hooks/exhaustive-deps
...
const handleCreate = async () => {
  try {
    ...
    navigate("/purchases");               // <- fica na tela de Compras
  } catch (err) {
    console.error(err);
    ...
    <div className="max-w-xl mx-auto bg-white p-4 pb-32">
      ...
      {step === 1 && (
        <>
          ...
          <div
            className="mb-2 w-full rounded-2xl border border-gray-200 px-4 py-3 cursor-pointer select-none"
            ...
            <ul className="mb-6 space-y-2">
              {extras.map((ex, i) => (
                <li key={i} className="flex items-center justify-between rounded-xl border border-gray-200 p-3">
                  ...
                  <button onClick={() => setExtras((prev) => prev.filter((_, idx) => idx !== i))} className="text-sm text-red-600">
                    Remover
                  </button>
                </li>
              ))}
